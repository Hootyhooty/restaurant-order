...
const createCheckoutSession = async (req, res) => {
  try {
    ...
    return res.json({ success: true, url: session.url, sessionId: session.id, transactionId: tx._id.toString() });
  } catch (error) {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const tx = transactionId
        ? await Transaction.findByIdAndUpdate(transactionId, { $set: update }, { new: true })
        : await Transaction.findOneAndUpdate({ stripeCheckoutSessionId: session.id }, { $set: update }, { new: true });
